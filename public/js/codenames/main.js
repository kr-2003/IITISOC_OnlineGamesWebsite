const socket = io("https://control-x.herokuapp.com/");
...
endTurnButton.addEventListener("click", () => {
  let user = userIDs[socket.id];
  if (
    ((currentGame.redTurn && currentGame.redTeam.includes(user)) ||
      (currentGame.blueTurn && currentGame.blueTeam.includes(user))) &&
    !currentGame.win
  ) {
...
socket.on("clickGamePiece", (gamePieceID, userID) => {
  ...
  if (
    (redTeam.includes(user) && currentGame.redTurn) ||
    (blueTeam.includes(user) && currentGame.blueTurn)
  ) {
